...
//Changed depending on the simulation orientation.
  ...
  for (int i = 0; i < 152; i++) {
    std::cout
        << i
        << " "
        << molecules->at(i).getMoleculeTrajectory()->get_trajectory_array()
            ->size()
        << std::endl;
  }
...
//average using the number of frames in the simulation.
  ...
  for (int i = 0; i < number_of_molecules; i++) {
    ...
    for (int j = 0; j < number_of_frames; j++) {
      ...
      angleholder[i] += molecules->at(i).getMoleculeTrajectory()
...
//First legendre sums
  float x0xt_sum, y0yt_sum = 0.0, z0zt_sum = 0.0;
  //Second legendre sums
  float x0xt_squared_sum = 0.0, y0yt_squared_sum = 0.0, z0zt_squared_sum = 0.0;
  float x0y0xtyt_sum = 0.0, x0z0xtzt_sum = 0.0, y0z0ytzt_sum = 0.0;

  //Then, declare arrays for the output results vs t
  std::vector<float> first_x, first_y, first_z;
  std::vector<float> second_xsquare, second_ysquare, second_zsquare;
  std::vector<float> second_xy, second_xz, second_yz;
...
//      std::cout << "j= " << j << " " << "x0xt_sum" << x0xt_sum << std::endl;
      x0xt_sum = x0xt_sum * (1.0 / (float)(number_of_frames - j));
//      std::cout << "j= " << j << " " << "x0xt_sum" << x0xt_sum << std::endl;
      y0yt_sum = y0yt_sum * (1.0 / (float)(number_of_frames - j));
      z0zt_sum = z0zt_sum * (1.0 / (float)(number_of_frames - j));
      x0xt_squared_sum = x0xt_squared_sum * (1.0 / (float)(number_of_frames - j));
      y0yt_squared_sum = y0yt_squared_sum * (1.0 / (float)(number_of_frames - j));
      z0zt_squared_sum = z0zt_squared_sum * (float)(1.0 / (number_of_frames - j));
      x0y0xtyt_sum = x0y0xtyt_sum * (1.0 / (float)(number_of_frames - j));
      x0z0xtzt_sum = x0z0xtzt_sum * (1.0 / (float)(number_of_frames - j));
      y0z0ytzt_sum = y0z0ytzt_sum * (1.0 / (float)(number_of_frames - j));
  ...
  for (int i = 0; i < number_of_frames; i++) {
//    std::cout << "i " << i << " " << first_x.at(i) << std::endl;
    first_x.at(i) = first_x.at(i) * (1.0 / (float)number_of_molecules);
    first_y.at(i) = first_y.at(i) * (1.0 / (float)number_of_molecules);
    first_z.at(i) = first_z.at(i) * (1.0 / (float)number_of_molecules);
    second_xsquare.at(i) = second_xsquare.at(i) * (1.0 / (float)number_of_molecules);
    second_ysquare.at(i) = second_ysquare.at(i) * (1.0 / (float)number_of_molecules);
    second_zsquare.at(i) = second_zsquare.at(i) * (1.0 / (float)number_of_molecules);
    second_xy.at(i) = second_xy.at(i) * (1.0 / (float)number_of_molecules);
    second_xz.at(i) = second_xz.at(i) * (1.0 / (float)number_of_molecules);
    second_yz.at(i) = second_yz.at(i) * (1.0 / (float)number_of_molecules);
  }
  ...
  for (int i = 0; i < number_of_frames; i++) {
    ...
    second_legendre=1.5*(second_xsquare.at(i) + second_ysquare.at(i) + second_zsquare.at(i) + (2*second_xy.at(i))+ (2*second_xz.at(i))+(2*second_yz.at(i)))-0.5;
